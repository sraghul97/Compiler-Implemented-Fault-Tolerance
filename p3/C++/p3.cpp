...
void CfgVerification(Function *FunctionPointer, Module *M)
{
  //CFG Verification
  ...
  { 
      ...
      if (succ_begin(BasicBlockPointer) != succ_end(BasicBlockPointer))  //if there is any successor
      {
        ...
        if (BranchInst *Branch = dyn_cast<BranchInst>(TerminalInstruction)) 
        {
            ...
            else if (Branch->getNumSuccessors() == 1)                         //Direct branch
            {
    ...
    { 
      ...
      for (BasicBlock *PredPointer : predecessors(BasicBlockPointer)) 
      {
        ...
        else // Null data from DiffMap
        {
            ...
            Value *XorResult = Builder.CreateXor(DestMap[PredPointer], DestMap[BasicBlockPointer]);
            SWFTAdded++;
...
void CodeReplication(Function *FunctionPointer, Module *M)
{
...
static void SoftwareFaultTolerance(Module *M) 
{
  ...
  for(std::vector<Function*>::iterator it = flist.begin(); it !=flist.end(); it++)
  {
    ...
    CfgVerification(*it, M);
    CodeReplication(*it, M);   
  }
}
